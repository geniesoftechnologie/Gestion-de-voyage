...
public class VolDao extends Dao <Vol> {
    ...
    public Vol rechercher(long id) {
        ...
        try {
            ...
            if(result.next() ){
                ...
                v.setOption(result.getString("opt"));
            }
    ...
    public void inserer(Vol v) {
        ...
        try {
            PreparedStatement prepare = this.connect.prepareStatement("INSERT INTO vol(id_avion, id_aeroport, nom_aeroport, datedepart, datearrivee, heurdepart, heurarrivee, prix, nbplacereserve, id_aeroport_ar, nom_aeroport_ar, duree, etat, opt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            prepare.setString(14, v.getOption());
    ...
    public void modifier(Vol v, long id) {
        SimpleDateFormat amj = new SimpleDateFormat("yyyy-MM-dd");
        try {
            PreparedStatement prepare = this.connect.prepareStatement("UPDATE vol SET id_avion = ? , id_aeroport = ?, nom_aeroport = ?, datedepart = ?, datearrivee = ?, heurdepart = ?, heurarrivee = ?, prix = ?, nbplacereserve = ?, id_aeroport_ar = ?, nom_aeroport_ar = ?, duree = ?, etat = ?, opt = ? WHERE id ="+id);
            prepare.setInt(1, v.getId_avion());
            prepare.setInt(2, v.getId_aeroport());
            prepare.setString(3, v.getNom_aero());
            prepare.setString(4, amj.format(v.getDatedepart()));
    ...
    public List<Vol> liste() {
        ...
        try {
            ...
            obj.setOption(result.getString("opt"));
            r.add(obj);
            obj = new Vol(); 
            }
    ...
    public List<Vol> listesimple() {
        List<Vol> r = new ArrayList();
        Vol obj = new Vol();
        ResultSet result;
        try {
            result = this.connect.createStatement().executeQuery("SELECT * FROM vol where opt='simple'");
            while(result.next()){
            obj.setId(result.getInt("id"));
            obj.setId_avion(result.getInt("id_avion"));
            obj.setId_aeroport(result.getInt("id_aeroport"));
    ...
    public List<Vol> listehadj() {
        List<Vol> r = new ArrayList();
        Vol obj = new Vol();
        ResultSet result;
        try {
            result = this.connect.createStatement().executeQuery("SELECT * FROM vol where opt='hadj'");
            while(result.next()){
            obj.setId(result.getInt("id"));
            obj.setId_avion(result.getInt("id_avion"));
            obj.setId_aeroport(result.getInt("id_aeroport"));
